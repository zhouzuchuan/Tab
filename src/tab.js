 ...
 * @Modified Time: 2016-05-13 17:43:35
 */
        ...
        _init: function () {
            ...
            elem.btn.forEach(function (element, index) {
                var val = element.getAttribute(daSer.btn),
                    dl = dealName(val);
                if (getType(self.tracePacket[dl.trace]) !== 'object') {
                    self.tracePacket[dl.trace] = {};
                }
                if (getType(self.tracePacket[dl.trace].sub) !== 'array') {
            ...
            // 处理运动方向
            var defaultDircetion = self.options.direction;
