 ...
 * @Modified Time: 2016-05-09 16:14:10
 */
    ...
    Tab.prototype = {
        ...
        _init: function () {
            var self = this,
                ...
                btn = '[' + daSer.btn + ']',
                main = '[' + daSer.main + ']',
                elem = coll.elements = this._getDom(),
                elemBtn = elem.btn;

            for (var i = 0, len = elemBtn.length; i < len; i += 1) {
                var val = elemBtn[i].getAttribute(daSer.btn);
                if (getType(coll.packetIndex[dealName(val).trace]) !== 'array') {
                    coll.packetIndex[dealName(val).trace] = [];
                }
                coll.packetIndex[dealName(val).trace].push(i);
                coll.key[i] = dealName(val).key;
                coll.trace[i] = dealName(val).trace;
                coll.all[i] = val;
                ...
                this.info[i] = {
                    node: elemBtn[i],
                    all: val,
                    key: dealName(val).key,
                    trace: dealName(val).trace,
                    current: false,
                    timer: null
                };
            ...
            // 设置默认显示
            if (getType(opt.defaultPos) === 'object') { /* 设置多个跟踪的默认显示 */
                ...
                //     console.log(getTraceVal, traceArr)
                // }
                self.info.forEach(function (element, index) {
                    // 获取设置的数字
                    var getTraceVal = opt.defaultPos[element.trace],
                    // 获取对应跟踪的下标分组
                        traceArr = coll.packetIndex[element.trace];
                    // 如果设置的默认不存在或者分组存在但没有指定默认 则默认显示第一个
                    self.set(isUndefined(traceArr[getTraceVal]) || isUndefined(getTraceVal) ? traceArr[0] : traceArr[getTraceVal]);
                });
            } else if (getType(opt.defaultPos) === 'number') { /* 共用同一个默认显示 */
                ...
                // 没有设置 则默认设置所有分组的第一个
                for (var props in coll.packetIndex) {
                    this.set(coll.packetIndex[props][0]);
                }
            }
            ...
            if (getType(opt.eventType) === 'object') {
                // for (var j = 0, all = coll.trace.length; j  < all; j += 1) {
                //     var eventType = opt.eventType[coll.trace[j]];
                //     if (!isUndefined(eventType)) { /* 设置分组事件方式 */
                //         switch (eventType) {
                //             case 'hover':
                //                 elemBtn.forEach(function (element, index) {
                //                     if (index === j) {
                ...
                self.info.forEach(function (element, index) {
                    // 获取设置默认事件的跟踪值
                    var eventType = opt.eventType[element.trace];
                    if (!isUndefined(eventType)) { /* 设置分组事件方式 */
                        switch (eventType) {
                            case 'hover':
                                element.node.onmouseover = handle();
                                break;
                            default:
                                element.node.onclick = handle();
                                break;
                        }
                    } else { /* 没有设置分组事件方式 执行默认 */
                        element.node.onclick = handle();
            ...
            function handle () {
                return function (e) {
                    var val = this.getAttribute(daSer.btn),
        ...
        set: function (idx) {
            var coll = this.collection,
                ...
                daSer = this._dealSelector(),
                elemBtn = coll.elements.btn,
                elemMain = coll.elements.main,
                dealCur = this._dealClass()[traceValue];
            // 判断当前状态是否选中
            if (coll.current[idx]) return;
            this.options.currentWill.call(this);
            // 复位
            elemBtn.forEach(function (element, index) {
                if (coll.trace[index] === traceValue) {
                    removeClass(dealCur.btn, element);
                    removeClass(dealCur.main, elemMain[index]);
                    coll.current[index] = false;
                }
            });
            // 设值
            addClass(dealCur.btn, elemBtn[idx]);
            addClass(dealCur.main, elemMain[idx]);

            coll.current[idx] = true;
            this.options.currentDid.call(this);
        },
        _getDom: function () {
            ...
            if (btnElem.length !== mainElem.length) {
                console.log('您设置的选择器数量不对应，请核对下数量！！！');
            }
            return {
                btn: btnElem,
                main: mainElem
        ...
        _dealClass: function () {
            ...
            if (getType(opt.currentClass) === 'object') {
                ...
                for (var j in this.collection.packetIndex) {
                    result[j] = dealArray(opt.currentClass);
                }
            }
            return result;
        }
